...
void ExchangeReceiverBase::waitAllAsyncConnDone()
{
    std::unique_lock lock(mu);
    auto pred = [&] {
...
void ExchangeReceiverBase::connectionAsyncDone(
    bool meet_error,
    const String & local_err_msg,
    const LoggerPtr & log)
{
    connectionDoneImpl(meet_error, local_err_msg, log, async_conn_num, "async");
}

void ExchangeReceiverBase::connectionDoneImpl(
    bool meet_error,
    const String & local_err_msg,
    const LoggerPtr & log,
    Int32 & conn,
